(function() {
	...
	eGarageApp.controller("ordersByDurationCtrl", ordersByDurationCtrl);
	ordersByDurationCtrl.$inject = [
		"$scope",
		"$state",
		"$sce",
		"$filter",
	...
	function ordersByDurationCtrl($scope, $state, $sce, $filter, dpConfFactory, restAPIFactory, isArResFactory) {
		// Objects decleration =>>
		// =======================
		...
		$scope.searchFormSubmit = function() {
			...
			restAPIFactory("GET", "assets/js/custom/model.json", {}).post(function(jsonData) {
				if(jsonData.statusText === "OK") {
					$scope.theadAr = jsonData.data[0].ordersByDurationTHeadAr;
		...
		$scope.printReport = function() {
			...
			pdfTpl += '<html>';
				pdfTpl += '<head>';
					pdfTpl += '<title>eGarage_Reports_OrdersByTerms</title>';
